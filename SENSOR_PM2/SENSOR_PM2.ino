...
boolean newData = false, leyendo2 = false, leyendo10 = false;
...
volatile uint32_t unPM10_InShared;
volatile uint32_t unPM2_InShared;
...
void calculateDustValue()
{
  ...
  static uint32_t unPM10_In;
  static uint32_t unPM2_In;
  static uint32_t unPM10_Time;
  static uint32_t unPM2_Time;
  ...
  */
  ...
  {
    PM2_Value = ((float)PM2_Output[0] / (samplerate * NoOfSamples * 10 ) * 600 / 7 + 250);
    PM10_Value = ((float)PM10_Output[0] / (samplerate * NoOfSamples * 10 ) * 600 / 7 + 250);
  }
 else
  {
...
void calcPM10()
{
  Serial.println("interrucion 10");
  if(digitalRead(PM10_IN_PIN) == LOW)
  {
    if(leyendo2 == true)
    {
      Serial.println("entro102-alto");
      Serial.printf("Acomulado: %d + %d \n", unPM2_InShared, (micros() - ulPM2_Start));
      unPM2_InShared = micros() - ulPM2_Start;
      Serial.println(unPM2_InShared);
      leyendo2 = false;
      bUpdateFlagsShared |= PM2_FLAG;
    }
    Serial.println("entro10-bajo");
    ulPM10_Start = micros();
    leyendo10 = true;
  }
  else if(digitalRead(PM2_IN_PIN) == HIGH && leyendo2 == true)
  {
    Serial.println("entro102-alto");
    Serial.printf("Acomulado: %d + %d \n", unPM2_InShared, (micros() - ulPM2_Start));
    unPM2_InShared = micros() - ulPM2_Start;
  ...
  else if(leyendo10 == true)
  {
    Serial.println("entro10-alto");
    Serial.printf("Acomulado: %d + %d \n", unPM10_InShared, (micros() - ulPM10_Start));
    unPM10_InShared = micros() - ulPM10_Start;
    Serial.println(unPM10_InShared);
    leyendo10 = false;
...
void calcPM2()
{
  Serial.println("interrucion 2");
  if(digitalRead(PM2_IN_PIN) == LOW)
  {
    if(digitalRead(PM10_IN_PIN) == LOW && leyendo10 == false)
    {
      Serial.println("entro210-bajo");
      ulPM10_Start = micros();
      leyendo10 = true;
    }
    else if(leyendo10 == false)
    {
      Serial.println("entro2-bajo");
      ulPM2_Start = micros();
      leyendo2 = true;
    }
  }
  ...
  else if(leyendo2 == true)
  {
    Serial.println("entro2-alto");
    Serial.printf("Acomulado: %d + %d \n", unPM2_InShared, (micros() - ulPM2_Start));
    unPM2_InShared = micros() - ulPM2_Start;
    Serial.println(unPM2_InShared);
    leyendo2 = false;
